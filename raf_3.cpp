    ...
    else if(step==1)        //start new game button
    {
        ...
        if(flag_w==1&& (xchange3)<(human_x+human_w)&&(xchange3)>human_x)
        {
            ...
            /*if(toggle_sfx)
                {
                    flag_c=1;
                    PlaySound(TEXT("w.wav"), NULL, SND_LOOP|SND_ASYNC);
                } */
            life_count--;
        }
...
//{
  if(flag==0)
  {
   ...
   if(womenib_gy>800) womenib_dgy=0;
...
//       iSetTimer(100, womenib_movement);         // N
      ...
      // iSetTimer(100, bossMovement);             // N
        ...
        iSetTimer(50000, ghost_come);
        iSetTimer(50000, ghost_change);
    ...
    //DWORD screenheight = GetSystemMetrics(SM_CYSCREEN);
     ...
     /*if(toggle_sfx&&flag_c)
    PlaySound(TEXT("w.wav"), NULL, SND_LOOP|SND_ASYNC); // F : game music*/
