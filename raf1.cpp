...
int flag3=0;  // N
int dragon_x=1500,dragon_y=600,move_dragon=0; // N : dragon
...
int score_count=00; // F : score int
...
int kill=0,temp=1,temp2=1; // F : for kill count ,temp for toggle
...
int step1mode=0; //S: SV  for input box activation in step 1, 1 means active, 0 means deactive
...
char buttonimages[][100]={"bin\\Debug\\pictures\\buttons\\startnewgame.bmp", "bin\\Debug\\pictures\\buttons\\options.bmp", "bin\\Debug\\pictures\\buttons\\highscores.bmp", "bin\\Debug\\pictures\\buttons\\creators.bmp",  "bin\\Debug\\pictures\\buttons\\help.bmp", "bin\\Debug\\pictures\\buttons\\exit.bmp", "bin\\Debug\\pictures\\buttons\\back.bmp", ""};  //S: char for button image location
...
void iDraw()
{
    ...
    iShowBMP(0,0,"bin\\debug\\images\\BACKGROUNDS\\1.bmp");
    if(flag==6) iShowBMP(0,0,"bin\\Debug\\images\\background\\boss background.bmp");  // N : boss
   // iShowBMP2(xchange3, ychange3,"bin\\debug\\images\\CHARACTERS\\hero.bmp",0);       //F :Hero main
      ...
      /* if(flag_g)
       {
        ...
        if(flag_w==1&& (xchange3)<(700)&&(xchange3)>600) // F : power life plus
        {
         ...
         if(kill==2) // F : sp power icon show
        {
            flag_sp=1;
            if(flag_sp)
            {
        ...
        if(kill==4||kill==6) // F : sp life icon show
        {
            flag_sl=1;
            if(flag_sl)
            {
        ...
        if(!game_over)
        {
            ...
            iText(3*WIDTH/8, 5*HEIGHT/8-120, "ENTER YOUR NAME, WORRIOR:",GLUT_BITMAP_TIMES_ROMAN_24);
            nameinputbox();
        }
        backbutton();
    }
    ...
    else if(step==3)        //High scores button
    {
        ...
        for(i=0; i<5; i++)
        {
            ...
            iText(5*WIDTH/8, ((3*HEIGHT)/4)-(i*40), struct_highscore, GLUT_BITMAP_TIMES_ROMAN_24);
        }
    }
    ...
    else if(step==4)        //Creators section
    {
        ...
        iShowBMP2(WIDTH/2-100, HEIGHT/2-100, creators[0], 0);
...
void iMouse(int button, int state, int mx, int my)
{
	if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
	{
        ...
        {
            if(mx>=3*WIDTH/8&&mx<=3*WIDTH/8+300&&my>=5*HEIGHT/8-160&&my<=5*HEIGHT/8-160+30&&game_over==0&&step1mode==0)
            {
                step1mode=1;
            }
        }
        ...
        if(step==1||step==2||step==3||step==4||step==5) // steps where back button will work, diverts to main page
        {
...
void iKeyboard(unsigned char key)
{
	if(step==1) //F: buttons declaring while in game
    {
        ...
        if(game_over==0&&step1mode==1) // S: for game over box name input
        {
            if(key == '\r')
            {
                step1mode=0;
...
//this function initializes game to initial level and values
void gameinitialize()
{
    /*game_over=1; // F : for isettimmer off
    score_count=0; // F : score int
    life_count=15; // F : life int
    xchange2, ychange2;   // F : for changing cloud pos
    flag_p=0;                      // F : pause or resume
    flag_g=1;                      // F : for ghost vanis
    xchange3=WIDTH/2, ychange3=HEIGHT-200;   // F : for changing hero pos
    hero_w=248,hero_h=159 ;//F : hero widgh and hight
    ghost_w=150,ghost_h=170 ;//F : ghost widgh and hight*/


        xchange=0, ychange=0;               //S : for changing ghost pos
        game_over=1; // F : for isettimmer off
        score_count=0; // F : score int
        life_count=10; // F : life int
...
void nameinputbox()
{
	...
	iRectangle(3*WIDTH/8, 5*HEIGHT/8-160, 300, 30);
	iSetColor(255, 255, 255);
	if(step1mode == 1)
	{
...
//{
  if(flag==0)
  {
   womenib_gx=womenib_gx+womenib_dgx;
   womenib_gy=womenib_gy+womenib_dgy;
   if(womenib_gx>1200)
   {
       womenib_dgx=-100;
       //move_womanib++;
...
//
   if(flag==6)
  {
   boss_x=boss_x+boss_dx;
   boss_y=boss_y+boss_dy;
   if(boss_x>1200)
   {
       boss_dx=-100;
       //move_womanib++;
